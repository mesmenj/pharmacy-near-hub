...
const PharmacyNearHubLanding = () => {
  ...
  const guardPharmacyIds: string[] = useMemo(() => {
  ...
  // important : permet de savoir si on a lancé une vraie recherche d'adresse
  ...
  const allPharmacies = useMemo(() => {
    return Object.values(dataPharmacy || {});
  }, [dataPharmacy]);

  const matchesPharmacySearch = (pharmacy: any, query: string) => {
  ...
  // 20 pharmacies autour de la position utilisateur
  const pharmaciesUserWithDistance = useMemo(() => {
    ...
    return allPharmacies
      .filter(
        (pharmacy: any) =>
          pharmacy &&
          typeof pharmacy.lat === "number" &&
          typeof pharmacy.lng === "number"
      )
      .map((pharmacy: any) => ({
        ...pharmacy,
        ...
        isOnGuard: guardPharmacyIds.includes(pharmacy.id),
      }))
      .sort((a: any, b: any) => a.distance - b.distance)
      .slice(0, 20);
  }, [location, allPharmacies, guardPharmacyIds]);

  // Recherche texte par nom/adresse sur toute la liste des pharmacies.
  const searchedPharmacies = useMemo(() => {
    const query = searchQuery.trim();

    if (!query) return [];

    return allPharmacies.filter((pharmacy: any) =>
      matchesPharmacySearch(pharmacy, query)
    );
  }, [allPharmacies, searchQuery]);

  // 20 pharmacies autour de l'adresse recherchée
  const pharmaciesAroundSearch = useMemo(() => {
    if (!searchCenter) return [];

    return allPharmacies
      .filter((pharmacy: any) => pharmacy?.lat && pharmacy?.lng)
      .map((pharmacy: any) => ({
        ...pharmacy,
        distance: getDistanceKm(searchCenter, {
          lat: pharmacy.lat,
          lng: pharmacy.lng,
        }),
        isOnGuard: guardPharmacyIds.includes(pharmacy.id),
      ...
      .sort((a: any, b: any) => a.distance - b.distance)
      .slice(0, 20);
  }, [searchCenter, allPharmacies, guardPharmacyIds]);

  // Résultats finaux affichés
  const displayedPharmacies = useMemo(() => {
    // cas 1 : recherche d'adresse réussie
    if (searchCenter) return pharmaciesAroundSearch;

    // cas 2 : recherche texte pharmacie
    if (searchQuery.trim()) {
    ...
    // cas 3 : affichage par défaut autour de l'utilisateur
    return pharmaciesUserWithDistance;
  }, [
    searchCenter,
    pharmaciesAroundSearch,
    searchQuery,
    searchedPharmacies,
    pharmaciesUserWithDistance,
    mapCenter,
    guardPharmacyIds,
  ]);
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    const matchingPharmacies = allPharmacies.filter((pharmacy: any) =>
      matchesPharmacySearch(pharmacy, query)
    );
    ...
    try {
      ...
      // si aucune pharmacie ne correspond, on considère la recherche comme un lieu/adresse
      setSearchCenter(coords);
      setMapCenter(coords);
      setShowMobileMap(true);
    } catch (err) {
      console.log("Adresse non trouvée, fallback sur recherche texte :", err);

      // si le géocodage échoue, on reste en recherche texte
      setSearchCenter(null);
      if (location) {
      ...
      <section className="container mx-auto px-4 py-8 md:py-12 lg:py-20">
        <div className="text-center max-w-3xl mx-auto mb-6 md:mb-8">
          ...
          <p className="text-sm sm:text-base md:text-lg lg:text-xl text-[#6B7280] mb-6 md:mb-10 px-4">
            ...
            ou autour d'une adresse de votre choix.
          </p>
            ...
            <div className="relative">
              <input
                ...
                onChange={(e) => {
                  ...
                  // si champ vidé, retour à la position utilisateur
            ...
            <div className="inline-flex items-center space-x-1 sm:space-x-2 bg-white rounded-lg px-3 sm:px-4 py-1.5 sm:py-2 shadow-sm border border-[#E5E7EB] max-w-full mx-2">
              ...
              <span className="font-medium text-xs sm:text-sm truncate">
                {`${place.suburb || place.city || ""}, ${
                  place.city_district || place.state || ""
                }`}
              </span>
            </div>
          )}
        ...
        <div className="relative mt-6 md:mt-10">
          <div className="lg:hidden flex justify-center space-x-2 mb-4">
            ...
            >
              Liste ({displayedPharmacies.length})
            </button>
            <button
              ...
              <div className="sticky top-0 bg-white p-3 sm:p-4 border-b border-gray-200 z-10">
                <div className="flex items-center justify-between">
                  <h3 className="font-semibold text-sm sm:text-base">
                    Pharmacies proches ({displayedPharmacies.length})
                  </h3>
